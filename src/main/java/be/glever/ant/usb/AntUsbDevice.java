...
public class AntUsbDevice implements Closeable {
    ...
    public void closeAllChannels() throws AntException {
        int maxChannels = capabilities.getMaxChannels();
    ...
    public Mono<Void> openBackgroundScanningChannel(AntChannel channel, int timeout) {
        ...
        return send(new NetworkKeyMessage(channelNumber, channel.getNetwork().getNetworkKey()), responseMatcher)
                ...
                // Special options for scanning
                .then(send(new EnableExtendedMessages(true), responseMatcher))
                .then(send(new SearchTimeoutMessage(channelNumber, timeout), responseMatcher))
                .then(send(new SetLowPrioritySearchTimeoutMessage(channelNumber, timeout), responseMatcher))
     ...
     *
     * @param requestMessage The message to send.
     * @param responseValidator A function to check if the response is valid, given the request. Throws IllegalStateException otherwise.
     * @return
